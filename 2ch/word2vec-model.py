...
def message_to_sentences(message, tokenizer, lemmas_bool, remove_stopwords=True):
    ...
    if type(message) == str and len(message) > 1:
        try:
...
def process_the_board(imageboard):
    ...
    #unlabeled = pd.read_csv('messages-'+imageboard+'.csv', header=0, delimiter="\t", quoting = csv.QUOTE_MINIMAL, skiprows=range(1, int(length)+1), error_bad_lines=False)
...
sentences += process_the_board('po')
# already_parsed
